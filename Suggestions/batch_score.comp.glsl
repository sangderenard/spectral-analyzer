...
float cam_beta_sum(uint base) {
    float t = 0.0;
    int nb = clamp(n_bands, 1, MAX_GPU_BANDS);
    for (int b = 0; b < nb; ++b) { float m = cam_verts[base + uint(CGV_BAND_BASE + b)]; if (m > 0.0) t += m; }
    return t;
}
float light_beta_sum(uint base) {
    float t = 0.0;
    int nb = clamp(n_bands, 1, MAX_GPU_BANDS);
    for (int b = 0; b < nb; ++b) { float m = light_verts[base + uint(LGV_BAND_BASE + b)]; if (m > 0.0) t += m; }
    return t;
}
    ...
    if (cam_n > 0u && light_n > 0u) {
        ...
        for (uint s = 0u; s < per_thread; ++s) {
            ...
            if (geom >= min_geom) {
                ...
                const float bc = cam_beta_sum(cb);
                const float bl = light_beta_sum(lb);
                const float e  = bc * bl * geom;    /* A: predicted contribution */
