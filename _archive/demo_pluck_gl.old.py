...
def _band_maps_cache_path(dx: float, n_segs: int) -> str:
    key = hashlib.md5(f"band_maps_v1_dx{dx:.6f}_segs{n_segs}".encode()).hexdigest()
...
class Renderer:
    ...
    def render(self):
        ...
        if a_pres > 0:
            ...
            # Clip to guitar cavity Z: back-plate (z=0) → top-plate (z=body_h).
            # This removes the PML padding layers above and below the body.
            glUniform1f(glGetUniformLocation(self._p_march, b'uZClipMin'), 0.0)
            glUniform1f(glGetUniformLocation(self._p_march, b'uZClipMax'), self.body_h)
...
def _parse_args():
    ...
    p.add_argument("--dx", type=float, default=0.010,
            ...
            if ev.type == QUIT:
                ...
                if ev.key == K_q:
                    ...
                    if new_segs != args.render_segs or abs(new_dx - args.dx) > 1e-6:
                        print(f"Rebuilding physics (segs={new_segs}, "
                              f"dx={new_dx:.4f}) ...", flush=True)
                        _sc, _ce, _info, _bh = _build_physics(
                            n_strings=6, dx=new_dx,
                            pad_cells=PAD_CELLS, n_pml=N_PML, n_segs=new_segs)
                        if _ce is not None:
                            scene, ce, args.render_segs, args.dx = _sc, _ce, new_segs, new_dx
                            body_h  = _bh or BODY_H
                            n_str   = ce.n_strings
                            paths   = _string_paths(outline, body_h,
                                                     n_strings=n_str, n_segs=new_segs)
                            total   = _excitation_total_samples(args.excitation)
                            total_frames = int(math.ceil(total / float(BLOCK_SAMPLES)))
                            R._frames = collections.deque(maxlen=max(1, total_frames + 8))
                            R.rebuild_physics(_ce, _info, paths, body_h)
                            panel.mark_applied('segs')
                            panel.mark_applied('dx')
                            print(f"  Grid {_info['Nx']}×{_info['Ny']}×{_info['Nz']}",
                                  flush=True)
                        else:
                            print("  Physics rebuild failed — keeping old params",
                                  flush=True)
