...
def test_every_completed_nonterminal_node_materializes_exactly_nine_children():
    tree = SparseSensorMipmap(3, maximum_depth=2)
    children = tree.complete_work(tree.root_id)
    assert len(children) == 9
    assert tree.complete_work(tree.root_id) == children
...
def test_fine_sample_keeps_global_uv_and_explicit_node_lineage():
    tree = SparseSensorMipmap(1, maximum_depth=1)
    child_id = tree.complete_work(tree.root_id)[8]
